        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl mx-auto text-center">
            <span className="inline-block px-4 py-2 rounded-full bg-primary/20 text-primary text-sm font-medium mb-6">
              {s.contact_hero_badge || "Contact Us"}
            </span>
            <h1 className="font-display text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-hero-foreground mb-6">
              {s.contact_hero_title1 || "Get in"} <span className="gradient-text">{s.contact_hero_highlight || "Touch"}</span>
            </h1>
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-16">
            ...
            <div className="bg-card rounded-2xl md:rounded-3xl p-6 md:p-8 lg:p-12 card-shadow border border-border">
              <div className="flex items-center gap-3 mb-6 md:mb-8">
                ...
                <div>
                  <h2 className="font-display text-xl md:text-2xl font-bold text-foreground">{s.contact_form_title || "Send a Message"}</h2>
                  <p className="text-muted-foreground text-xs md:text-sm">{s.contact_form_subtitle || "Fill out the form and we'll get back to you"}</p>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  <div>
                    ...
                    <Input name="name" value={formData.name} onChange={handleChange} placeholder="John Doe" required className="h-11 md:h-12" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-2">Phone Number</label>
                    <Input name="phone" value={formData.phone} onChange={handleChange} placeholder="+91 1234567890" required className="h-11 md:h-12" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-2">Email Address</label>
                  <Input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="john@example.com" required className="h-11 md:h-12" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-2">Service Required</label>
                  <select name="service" value={formData.service} onChange={handleChange} required className="w-full h-11 md:h-12 px-4 rounded-lg border border-input bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-ring">
                ...
                <Button type="submit" size="lg" className="w-full" disabled={isSubmitting}>
                  {isSubmitting ? "Sending..." : <>{s.contact_form_btn || "Send Message"}<Send className="w-4 h-4 md:w-5 md:h-5 ml-2" /></>}
                </Button>
              </form>
            </div>
            ...
            <div className="space-y-6 md:space-y-8">
              <div>
                <h2 className="font-display text-xl md:text-2xl font-bold text-foreground mb-3 md:mb-4">{s.contact_info_title || "Contact Information"}</h2>
                  ...
                  <div key={idx} className="flex gap-4 md:gap-5 p-4 md:p-6 rounded-xl md:rounded-2xl bg-muted/50 hover:bg-muted transition-colors">
                    ...
                    <div>
                      ...
                      {info.details.map((detail, i) => (
                        <p key={i} className="text-muted-foreground text-xs md:text-sm">{detail}</p>
                      ))}
                    </div>
                  </div>
                ))}
        ...
        <div className="container mx-auto px-4">
          <div className="bg-gradient-to-r from-primary/10 to-accent/10 rounded-2xl md:rounded-3xl p-6 md:p-8 lg:p-12 flex flex-col lg:flex-row items-center justify-between gap-6 md:gap-8">
            <div className="text-center lg:text-left">
              <h3 className="font-display text-xl md:text-2xl font-bold text-foreground mb-2">{s.contact_cta_title || "Need Immediate Assistance?"}</h3>
              <p className="text-muted-foreground text-sm md:text-base">{s.contact_cta_subtitle || "Call us directly for urgent tech support."}</p>
            </div>
